...
class Botão:
    ...
    #Definindo atributos principais da classe botão:
    def __init__(self,texto,largura_botão,alura_botão,posição): 

        #Outros atributos:
        self.pressed = False #Vai armazenar a condição de click do botão

        #Retângulo superior:
        self.top_rect = pygame.Rect(posição,(largura_botão,alura_botão)) #Criar o retângulo superior
        self.top_color = '#475F77' #Definir a cor desse retângulo

        #Segundo retângulo:
        self.bottom_rect = pygame.Rect(posição,(largura_botão,alura_botão)) #Criar o retângulo inferior
        self.bottom_color = '#354B5E' #Definir a cor desse retângulo

        #texto do botão:
        self.text_surf = fonte.render(texto,True,branco) #Criando texto em si
        self.text_rect = self.text_surf.get_rect(center = self.top_rect.center) #Colocando o texto centralizado no Rect
    ...
    def checar_click(self):
        posição_mouse = pygame.mouse.get_pos() #Obtendo a posição do mouse (x,y)
        if self.top_rect.collidepoint(posição_mouse): #Verificando se houve sobreposião entro o Rect e o mouse
            self.top_color = vermelho #Adicionando cor flutuante
        
            if pygame.mouse.get_pressed()[0]:
                self.pressed = True
        
            else:
                #Condição que vai fazer com que o código referente ao click no botão seja executado só 1 vez
                if self.pressed == True:
                    self.pressed = False
        else:
            self.top_color = azul #Removendo a cor flutuante
...
def tela_rank(largura_janela,altura_janela):
    ...
    while sair == False:
        ...
        for event in pygame.event.get():
            ...
            #Verifando se houve clique no botão Sair:
...
# Posiçao do jogador
jogadorX = 405 #25 (Padrão)
jogadorY = 27  #25 (Padrão)
jogadorMudançaX = 0
jogadorMudançaY = 0
...
while sair != True: #CÓDIGO REFERENTE AO MAPA 1
    ...
    if jogador.colliderect(chegada):
        ...
        time.sleep(6) 
        #-----------------------------------------------------
        jogadorX = 40 #25 (Padrão)
        jogadorY = 29 #60 (Padrão)
...
#-------------------------------------------------------------------------------------------------------------------------------------
        while sair != True: #Código do mapa 2
            ...
            if jogador.colliderect(chegada): #passa de mapa
                ...
                time.sleep(6) 
                #-----------------------------------------------------
                jogadorX = 420 #462 Padrão
                jogadorY = 462 #25 Padrão
                ...
                while sair != True: #Código do mapa 3
                    ...
                    if jogador.colliderect(chegada):
                        somChegada.play()
                        #chegadaX = 25 Reverter
                        #chegadaY = 25 Reverter
                        #mudouChegada = True  Reverter
                        ...
                        tela_rank(largura_janela,altura_janela)''' #Reverter
